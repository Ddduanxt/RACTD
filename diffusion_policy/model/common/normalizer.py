...
class SingleFieldLinearNormalizer(DictOfTensorMixin):
    ...
    def normalize(self, x: Union[torch.Tensor, np.ndarray]) -> torch.Tensor:
        ...
        # return _normalize_new(x, self.params_dict['input_stats'])

    def unnormalize(self, x: Union[torch.Tensor, np.ndarray]) -> torch.Tensor:
        return _normalize(x, self.params_dict, forward=False)
        # return _unnormalize_new(x, self.params_dict['input_stats'])
...
def _normalize(x, params, forward=True):
    ...
    # print(scale)
    # print(offset)
    # print(params['input_stats']['min'])
    # print(params['input_stats']['max'])
    # print(params['input_stats']['mean'])
    # print(params['input_stats']['std'])
    ## obs std: ensor([0.1757, 0.0637, 0.3438, 0.1957, 0.5548, 1.0510, 1.1583, 0.7963, 1.4802, 1.6540, 5.1086], device='cuda:0')
    ## action std: tensor([0.5368, 0.5815, 0.7573], device='cuda:0')
...
def _normalize_new(x, params, eps=1e-4):
    # assert 'scale' in params
    if isinstance(x, np.ndarray):
        x = torch.from_numpy(x)
    xmin = params['min']
    xmax = params['max']
    ## x: (B, H, Da)
    x = x.to(device=xmin.device, dtype=xmin.dtype)
    src_shape = x.shape
    x = x.reshape(-1, xmin.shape[0])
    # if forward:
    #     x = x * scale + offset
    # else:
    #     x = (x - offset) / scale
...
def _unnormalize_new(x, params, eps=1e-4):
    # assert 'scale' in params
    if isinstance(x, np.ndarray):
        x = torch.from_numpy(x)
    xmin = params['min']
    xmax = params['max']
    x = x.to(device=xmin.device, dtype=xmax.dtype)
    src_shape = x.shape
    x = x.reshape(-1, xmin.shape[0])
    # if forward:
    #     x = x * scale + offset
...
class SingleFieldLinearNormalizerNew(DictOfTensorMixin):
    avaliable_modes = ['limits', 'gaussian']
...
class GaussianNormalizer(DictOfTensorMixin):
    avaliable_modes = ['limits', 'gaussian']
    ...
    def __getitem__(self, key: str):
        return SingleFieldGaussianNormalizerNew(self.params_dict[key])

    def __setitem__(self, key: str , value: 'SingleFieldGaussianNormalizerNew'):
        self.params_dict[key] = value.params_dict
    ...
    def _normalize_impl(self, x, forward=True):
        if isinstance(x, dict):
            ...
            print('using gaussian in normalize impl')
            if '_default' not in self.params_dict:
                raise RuntimeError("Not initialized")
            for key, value in self.params_dict.items():
                print(key)
...
class SingleFieldGaussianNormalizerNew(DictOfTensorMixin):
    avaliable_modes = ['limits', 'gaussian']
    ...
    def normalize(self, x: Union[torch.Tensor, np.ndarray]) -> torch.Tensor:
        print('using gaussian normalizing')
        if isinstance(x, np.ndarray):
            x = torch.from_numpy(x)

        mean = self.params_dict['input_stats']['mean']
        std = self.params_dict['input_stats']['std']

        x = x.to(device=mean.device, dtype=mean.dtype)
        src_shape = x.shape
