...
from consistency_policy.reward_guided_student_maze.guided_ctm_policy_maze import GuidedCTMPUnetMazePolicy
...
class GuidedCTMWorkspaceMaze(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        self.model: GuidedCTMPUnetMazePolicy = hydra.utils.instantiate(cfg.policy)
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if not cfg.training.inference_mode:
                    ...
                    if len(rwd_losses) > 0:
                        rwd_loss = np.mean(rwd_losses)
                        step_log['rwd'] = rwd_loss
...
def main(cfg):
    workspace = GuidedCTMWorkspaceMaze(cfg)
    workspace.run()
